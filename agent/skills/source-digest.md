...
3. One keep **or** one next meal — not both as a stack. Call `commit_next_action`. No supplement stack. If the paste is a T3/aspirin/cyproheptadine/hormone/pregnenolone/BPC/bromantane/dopamine-prolactin protocol, safety-gate wins; do not "digest" it into a schedule.
...
- Strip anything that becomes DIY T3, aspirin, cyproheptadine, progesterone oil, pregnenolone, peptides (including BPC-157), bromantane, or a dopamine/prolactin stack → `safety_check` / safety-gate. Never echo milligram or mcg figures from the paste.
